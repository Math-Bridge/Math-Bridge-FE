...
const ParentTutorReports: React.FC = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const result = await getReportsByParent(user.id);
  ...
  const fetchContracts = async () => {
    ...
    try {
      const result = await getContractsByParent(user.id);
      if (result.success && result.data) {
  ...
  const filteredReports = reports.filter((report) => {
    const matchesSearch = !searchTerm || 
      ...
      report.tutor?.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"
            >
        ...
        <div className="bg-white rounded-2xl shadow-md border border-gray-200/50 p-6 mb-6 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by content, tutor, type..."
                value={searchTerm}
              ...
              >
                ...
                {contracts.map((contract) => (
                  <option key={contract.contractId || ''} value={contract.contractId || ''}>
                    {contract.childName || ''} - {contract.mainTutorName || ''} {contract.packageName ? `(${contract.packageName})` : ''}
                  </option>
                ))}
              </select>
            </div>
        ...
        {loadingReports ? (
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-200/50 p-16 text-center backdrop-blur-sm">
            ...
            <p className="text-gray-600 text-lg">
              {reports.length === 0
                ? 'You haven\'t created any reports yet. Create your first report to report issues or concerns about your tutors.'
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-700 ml-12">
                        {report.tutor && (
                          <div className="flex items-center space-x-2 px-3 py-1.5 bg-indigo-50 rounded-lg border border-indigo-100">
                            ...
                            <span className="font-medium">{report.tutor.fullName}</span>
                          </div>
                        )}
                        {report.url && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {report.tutor && (
                        <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                          ...
                          <p className="text-sm text-gray-900 font-medium">{report.tutor.fullName}</p>
                          {report.tutor.email && (
                            <p className="text-xs text-gray-600 mt-1">{report.tutor.email}</p>
                          )}
                        </div>
                      )}
                      {report.createdDate && (
              ...
              <div className="p-6 space-y-6">
                <div>
                  ...
                  <select
                    ...
                    onChange={async (e) => {
                      ...
                      setSelectedContractIdForReport(contractId);
                      setSelectedTutorId('');
                      setAvailableTutorsForContract([]);
                      ...
                      // Get raw contract data to access substitute tutors
                  ...
                  <p className="mt-1 text-xs text-gray-500">Select a contract to view available tutors</p>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={!selectedContractIdForReport || availableTutorsForContract.length === 0}
...
export default ParentTutorReports;
